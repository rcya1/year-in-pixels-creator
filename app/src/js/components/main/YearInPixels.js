...
export default class YearInPixels extends React.Component {
  ...
  selectCurrentDay = () => {
    ...
    this.setState({
      ...
      menuVisible: true,
      currentlySelected: [
        Math.floor(this.props.currentDay / 31),
        this.props.currentDay % 31,
      ],
    });
  };
  ...
  render() {
    let title = (
      <h1 className="display-5 mt-3">{this.props.year + " in Pixels"}</h1>
    );
    ...
    let board = (
      <Board
        ...
        invalidCellsDisplayType={
          this.props.boardSettings.invalidCellsDisplayType
        }
    ...
    let colorSchemeList = <ColorSchemeList {...colorSchemeListProps} />;
    ...
    let toolbar = (
      <div
        className={
          "mx-auto d-flex flex-row align-items-center " +
